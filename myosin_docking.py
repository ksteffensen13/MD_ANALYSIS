...
for i in range(1, 6):
    cmd.delete('all')
    cmd.load(f'{initial_file_directory}DOCKING_{initial_distance}A_{mutant}{i}_{state}_MINIMIZED_SD.pdb')
    #Pymol changes chain lettering, so take post-minimization structure and fix it
    cmd.alter(selection='chain H', expression="chain='M'")
    cmd.alter(selection='resn ADP', expression="chain='G'")
    cmd.alter(selection='resn PO4', expression="chain='H'")
    cmd.alter(selection='resn MG', expression="chain='I'")
    #to make all TPM  consistent across structures, we'll move the structure to the same point in Pymol
    ...
    cmd.alter_state(1, f'DOCKING_{initial_distance}A_{mutant}{i}_{state}_MINIMIZED_SD', f"x,y,z=x-{v[12]},y-{v[13]},z-{v[14]}")
    ...
    cmd.alter_state(1, f'DOCKING_{initial_distance}A_{mutant}{i}_{state}_MINIMIZED_SD',
                    f"x,y,z=x*{v[0]}+y*{v[3]}+z*{v[6]},x*{v[1]}+y*{v[4]}+z*{v[7]},x*{v[2]}+y*{v[5]}+z*{v[8]}")
    cmd.orient(selection='all')
    cmd.translate(vector=[0.0, 0.0, -0.5], selection=f'chain M', camera=0)
    cmd.save(f'{path_for_output}DOCKING_{docking_distance}A_{mutant}{i}_{state}.pdb')
    cmd.delete('all')
