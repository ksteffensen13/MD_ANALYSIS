...
for i in range(1, 6):
    cmd.delete('all')
    cmd.load(f'{initial_file_directory}{mutant}{i}_{state}.pdb')
    ...
    cmd.alter_state(1, f'{mutant}{i}_{state}', f"x,y,z=x-{v[12]},y-{v[13]},z-{v[14]}")
    ...
    cmd.alter_state(1, f'{mutant}{i}_{state}', f"x,y,z=x*{v[0]}+y*{v[3]}+z*{v[6]},x*{v[1]}+y*{v[4]}+z*{v[7]},x*{v[2]}+y*{v[5]}+z*{v[8]}")
    cmd.orient(selection='all')
    cmd.translate(vector=[0.0, 0.0, 3.0], selection=f'chain M', camera=0)
    cmd.save(f'{path_for_output}DOCKING_3.0A_{mutant}{i}_{state}.pdb')
    cmd.delete('all')
